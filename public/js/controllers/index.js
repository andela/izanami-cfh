...
.controller('IndexController', [
  '$rootScope', '$http', '$scope', 'Global', '$location', 'socket',
  'game', 'AvatarService', 'playerSearch',
  ($rootScope, $http, $scope, Global, $location, socket, game,
     AvatarService, playerSearch) => {
    $scope.global = Global;
        ...
        .success((response) => {
          if (response.length) {
            ...
            $scope.friends = Object.keys(response[0]).map((friend) => {
              if (friend !== '_id') {
                const friendData = {
                  email: response[0][friend].email,
                  name: response[0][friend].name,
                  avatar: response[0][friend].avatar || '/img/anonymous.png',
                  id: friend };
                return friendData;
              }
              return null;
      ...
      $http.get(`/api/friends/status/${user}/${window.user.id}`)
        .success((response) => {
          ...
          if (response.message === 'Friend request sent') {
            $scope.cancelReq = true;
          } else {
            $scope.cancelReq = false;
          }
          $scope.searchUsername = $scope.inviteeUserName;
        });
    };
    ...
    $scope.addToList = (id, email) => {
      if (!$scope.customGame) {
        $scope.customGame = true;
      }
      $scope.sentInvite.push(id);
      socket.emit('addFriend', { id, email });
    };

    $scope.removeFromList = (id, email) => {
      const index = $scope.sentInvite.indexOf(id);
      if (index !== -1) {
        $scope.sentInvite.splice(index, 1);
        socket.emit('removeFriend', { id, email });
      }
      if ($scope.sentInvite.length === 0) {
        $scope.customGame = false;
      }
    };

    $scope.inviteSent = (id) => {
      return $scope.sentInvite.includes(id);
    };
    ...
    $scope.sendRequest = (user) => {
      $http.post('/api/friends/request', { id: user, sender: window.user },
        ...
        .success((response) => {
          ...
          $scope.friendshipStatusMessage = 'Friend request sent';
          $scope.cancelReq = true;
    ...
    $scope.cancelFriendReq = (sender) => {
      $http.post('/api/friends/request/reject', { user: sender, sender: window.user.id },
      { headers: { 'Content-Type': 'application/json' } })
      .success(() => {
        socket.emit('makeBroadcast', { message: 'refreshNotification', user: sender });
        $scope.cancelReq = false;
        $scope.friendshipStatus = false;
        $scope.friendshipStatusMessage = 'Add as friend';
      });
    };
